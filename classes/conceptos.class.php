...
class Conceptos extends Module
{
    ...
    public function historial_pagos($orderBy = "fecha_cobro")
    {
        ...
        $sql = "SELECT pagos.pago_id, pagos.course_id, pagos.alumno_id, pagos.concepto_id, pagos.fecha_cobro, if(pagos.status = 3, DATE_ADD(pagos.fecha_limite, INTERVAL pagos.tolerancia DAY), pagos.fecha_limite) as fecha_limite, pagos.total, pagos.iva, pagos.subtotal, CASE WHEN pagos.status = 1 THEN '<span class=\"badge badge-warning\">Pendiente</span>' WHEN pagos.status = 3 THEN '<span class=\"badge badge-info\">Prórroga</span>' WHEN pagos.status = 4 THEN '<span class=\"badge badge-outline-danger\">Condonado</span>' ELSE '<span class=\"badge badge-primary\">Pagado</span>' END AS status_btn, pagos.status, pagos.descuento, pagos.beca, pagos.archivo, pagos.tolerancia, pagos.periodo, conceptos.nombre AS concepto_nombre, pagos.indice, recibos.id as recibo
        FROM pagos 
    ...
    public function guardarCobroTarjeta($marca_tarjeta, $referencia3d, $correo, $nombre, $apellido, $codigo_postal, $celular, $tipo_tarjeta, $numero_tarjeta, $fecha_exp, $codigo_seguridad, $session_id)
    {
        $sql = "INSERT INTO cobros_tarjeta(session_id, pago_id, monto, marca_tarjeta, referencia3d, correo, nombre, apellido, codigo_postal, celular, tipo_tarjeta, estatus, numero_tarjeta, fecha_exp, codigo_seguridad, created_at, updated_at) VALUES('" . $session_id . "', " . $this->pagoId . ", " . $this->monto . ", '" . $marca_tarjeta . "', '" . $referencia3d . "', '" . $correo . "', '" . $nombre . "', '" . $apellido . "', '" . $codigo_postal . "', '" . $celular . "', '" . $tipo_tarjeta . "', 'Auth', '" . $numero_tarjeta . "', '" . $fecha_exp . "', '" . $codigo_seguridad . "', NOW(), NOW())";
        $this->Util()->DB()->setQuery($sql);
        $resultado = $this->Util()->DB()->InsertData();
        return $resultado;
    }
    ...
    public function getCobroTarjeta($referencia3d = 'NULL')
    {
        $sql = "SELECT cobros_tarjeta.*, pagos.alumno_id AS userId 
                    FROM cobros_tarjeta 
                        INNER JOIN pagos 
                            ON cobros_tarjeta.pago_id = pagos.pago_id 
                    WHERE cobros_tarjeta.deleted_at IS NULL AND cobros_tarjeta.referencia3d = '" . $referencia3d . "'";
        if($this->cobroTarjetaId > 0)
        {
            $sql = "SELECT cobros_tarjeta.*, pagos.alumno_id AS userId 
                        FROM cobros_tarjeta 
                            INNER JOIN pagos 
                                ON cobros_tarjeta.pago_id = pagos.pago_id 
                        WHERE cobros_tarjeta.deleted_at IS NULL AND cobros_tarjeta.id = " . $this->cobroTarjetaId;
        }
        $this->Util()->DB()->setQuery($sql);
        $result = $this->Util()->DB()->GetRow();
        return $result;
    ...
    /**
     * Pasa un pago al estatus 4(pago condonado)
     */
    function condonar() {
        $sql = "UPDATE pagos SET status = 4 WHERE pago_id = {$this->pagoId}";
        $this->Util()->DB()->setQuery($sql); 
