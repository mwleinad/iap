...
var App = function() {
    ...
    var handleBootstrapConfirmation = function() {
        ...
        jQuery('[data-toggle=confirmation]').confirmation({ btnOkClass: 'btn btn-sm btn-success', btnCancelClass: 'btn btn-sm btn-danger'});
    }
    
    // Handles Bootstrap Accordions.
    var handleAccordions = function() {
        jQuery('body').on('shown.bs.collapse', '.accordion.scrollable', function(e) {
            App.scrollTo(jQuery(e.target));
        });
    };
   ...
   var handleHeight = function() {
    ...
    return {
        ...
        alert: function(options) {
            ...
            if (options.reset) {
                jQuery('.custom-alerts').remove();
            }

            if (!options.container) {
                if (jQuery('.page-fixed-main-content').size() === 1) {
                    jQuery('.page-fixed-main-content').prepend(html);
                } else if ((jQuery('body').hasClass("page-container-bg-solid") || jQuery('body').hasClass("page-content-white")) && jQuery('.page-head').size() === 0) {
                    jQuery('.page-title').after(html);
                } else {
                    if (jQuery('.page-bar').size() > 0) {
                        jQuery('.page-bar').after(html);
                    } else {
                        jQuery('.page-breadcrumb, .breadcrumbs').after(html);
                    }
                }
            } else {
