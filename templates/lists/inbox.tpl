	...
	<table class="table table-sm table-bordered table-hover">
		...
		<tbody>
			{foreach from=$lstMsj item=subject}
				<tr>
					<td class="pointer">
						<input type="checkbox" name="checkInbox[]" id="check_{$subject.chatId}" value="{$subject.chatId}">
						{if $subject.favorito eq 'si'}
							<i class="far fa-star"></i>
						{/if}
						<a href="{$WEB_ROOT}/view-inbox/id/{$subject.courseModuleId}/cId/{$subject.chatId}&or={$or}"
							style="text-decoration:none">
							{if $subject.leido eq 'no' and $tipo eq 'entrada'}
								<b>
								{/if}
								<label style="text-transform:capitalize;" class="text-muted">{$subject.nombre}
									{$subject.paterno} {$subject.materno}</label>
								{if $subject.leido eq 'no' and $tipo eq 'entrada'}
								</b>
							{/if}
						</a>
					...
					<td class="pointer">
						<a href="{$WEB_ROOT}/view-inbox/id/{$subject.courseModuleId}/cId/{$subject.chatId}"
							style="text-decoration:none">
							<i>
								{if $subject.leido eq 'no' and $tipo eq 'entrada'}
