<form action="{$WEB_ROOT}/ajax/acta-examen.php" method="POST" target="_blank">
    ...
    <div class="row">
        ...
        <div class="form-group col-md-4">
            <label for="nombreJefe">Nombre del Jefe de la Oficina</label>
            <input type="text" name="nombreJefe" id="nombreJefe" class="form-control" onkeyup="this.value = this.value.toUpperCase()" required />
        </div>  
    </div> 
    <div class="row">
    ...
    $("body").on("change", "#student", function(){
        ...
        $.ajax({
            ...
            if (response.status) {
                ...
                $("#nombreJefe").val(response.data.boss); 
            }else{
                $("#fecha").val("");
                $("#folio").val("");
                $("#noActa").val("");
